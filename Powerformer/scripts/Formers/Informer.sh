...
for pred_len in 1024 #96 192 336 720
do
  if [ "$NS" -eq "1" ]
  then
  ...
  python3 -u run_longExp.py \
    --random_seed $random_seed \
    --is_training 1 \
    --root_path /scratch/khegazy/datasets/weather/ \
  ...
  python3 -u run_longExp.py \
      --random_seed $random_seed \
      --is_training 1 \
      --root_path /scratch/khegazy/datasets/electric_transformer_temperature_small/ \
      --data_path ETTh1.csv \
